      ...
      column(10, offset = 1,
          fluidRow(
              ...
              column(4,
                  ...
                  cycIHC utilizes sequential rounds of colorimetric immunostaining and imaging on the same tissue section for quantitative mapping of location and number of cells of interest. Amino ethyl carbazol (AEC) is a 
  ...
  fluidRow(
      column(10, offset = 1,
          fluidRow(
              ...
              column(4,
                  tags$p(class = "help_text", "To analyze single cell expression data from a cyclic IHC experiment, near pixel perfect registration 
                  across all rounds of staining is crucial. To that end, first rounds of staining are registered 
                  at a lower resolution that captures the presence and shape of the gross tissue section. Registration at this level is more robust to large 
                  scale shifts in the tissue position within the imaged area, as well as loss of tissue due to repeated handling of slides required for cyclic staining.")
              )
          )
      )
  ),
  ...
  fluidRow(
      column(10, offset = 1,
          fluidRow(
              ...
              column(4,
                  tags$p(class = "help_text", "For each tile containing sufficient tissue for analysis, a keypoints detection and matching registration strategy is used to
                  achieve pixel-level registration of individual cells. In each image, keypoints are detected and their coordinates and descriptors are saved. A transformation
                  is computed that minimizes the distance between matching keypoints and maximizes the number of keypoint matches that produce this transformation. Each image 
                  is first registered to its corresponding round zero image. Registering all images in a series back to the original round image eliminates the danger of a ‘drift’ 
                  in registration where registration errors may be propagated down the line of registrations.")
              )
          )
      )
  ),
  ...
  fluidRow(
      column(10, offset = 1,
          fluidRow(
             ...
             column(4,
                  tags$p(class = "help_text", "Marker expression values on each segmented cell are quantified and assembled into an expression table.  Cells can now be plotted
                         by their coordinates with marker expression displayed as heat.")
             ) 
          )
      )
  ),
  ...
  fluidRow(
      column(10, offset = 1,
          fluidRow(
              ...
              column(4,
                  tags$p(class = "help_text", "Log odds values represent the relative chance that cells of two populations will interact with each other given their 
                  frequency and distribution within the dataset. A log odds value greater than 1 indicates a greater than random chance interaction frequency, and 
                  conversely a log odds value less than -1 indicates the interaction frequency is less than what would be expected by random chance")
              ) 
          )
      )
  ),
